...
describe("RepoCard", () => {
  ...
  test("Should render properly - mount card with data passed via props and request user data", async () => {
    ...
    axios.get = vi.fn().mockResolvedValue({ data: githubUser });
    ...
    waitFor(async () => {
      ...
      if (githubUser?.bio) {
        expect(await screen.findByText(githubUser?.bio)).toBeInTheDocument();
      }

      expect(await screen.findByText(githubUser.company)).toBeInTheDocument();
    });
  });
  ...
  test("Should open user info", async () => {
    ...
    if (githubUser?.email) {
      expect(screen.queryByText(githubUser?.email)).not.toBeInTheDocument();
    }
    ...
    waitFor(async () => {
      ...
      expect(await screen.findByText(githubUser.name)).toBeInTheDocument();
      expect(await screen.findByText(githubUser.company)).toBeInTheDocument();
      expect(await screen.findByText(githubUser.login)).toBeInTheDocument();

      if (githubUser?.bio) {
        expect(await screen.findByText(githubUser?.bio)).toBeInTheDocument();
      }
      if (githubUser?.email) {
        expect(await screen.findByText(githubUser?.email)).toBeInTheDocument();
      }
    });
  });
  ...
  test("Should open a new tab when click on icon to access repository", async () => {
    ...
    const openPageSpy = vi.spyOn(window, "open");
